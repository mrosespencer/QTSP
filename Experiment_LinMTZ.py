# This program runs the experimentation for the MTZ formulation of the linearized quadratic traveling salesman problem. We then test methods to linearize the quadratic objective function. Those linearizations are: replacing the quadratic term with a binary variable, the classical linearization using continuous variables, the McCormick envelopes, base 2 and base 10linearizations. All linearized models are found in the sub folder LiinMTZ. We test these linearization against the non-linear form. We also test the linear relaxation. The outputs for this program are the latex file that produces the tables in my report, as well as simple text files of the objective value, gap value, Gurobi status, and the tour of the TSP. It also outputs the Gurobi log files.
