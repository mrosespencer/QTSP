...
for p in range(8):
    ...
    for n in size:
        ...
        for t in range(trials):
            ...
            name = qname + "-" + str(1)
            q1 = QMod.half(q, e)
            obj[1,count], time[1,count], x, gap[1,count], status[1,count] = QuadMTZ.SolveTSP(n, c, q1, name, adj, presolve)
            ...
            obj[2,count], time[2,count], x, gap[2,count], status[2,count] = QuadMTZ.SolveTSP(n, c, q2, name, adj, presolve)
            tour[2,count], full[2,count] = VerifyTour.check(x, n)

            # Make Q positive semi-definite
            name = qname + "-" + str(3)
            q3 = QMod.plusm(q, e, m)
            obj[3,count], time[3,count], x, gap[3,count], status[3,count] = QuadMTZ.SolveTSP(n, c, q3, name, adj, presolve)
            ...
            obj[4,count], time[4,count], x, gap[4,count], status[4,count]  = QuadMTZ.SolveTSP(n, c, q4, name, adj, presolve)
            ...
            obj[5,count], time[5,count], x, gap[5,count], status[5,count] = QuadMTZ.SolveTSP(n, cr, qr, name, adj, presolve)
            tour[5,count], full[5,count] = VerifyTour.check(x, n)

            # Make QR into symmetric matrix
            name = qname + "-" + str(6)
            q6 = QMod.half(qr, e)
            obj[6,count], time[6,count], x, gap[6,count], status[6,count] = QuadMTZ.SolveTSP(n, cr, q6, name, adj, presolve)
            ...
            obj[7,count], time[7,count], x, gap[7,count], status[7,count] = QuadMTZ.SolveTSP(n, cr, q7, name, adj, presolve)
